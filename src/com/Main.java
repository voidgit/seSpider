...
class Main {
    ...
    private static void addFilterPrio(String regexp, Integer i) {
        Main.FiltersPrio.put(regexp, i);
    }

    private static void addFilterPath(String regexp, String newValue) {
        Main.FiltersPath.put(regexp, newValue);
    }

    private static void addUrl(String Url, int distance) {
        if ( distance <= MaxDepth) {
            seURL seurl = new seURL(Url, FiltersPrio, distance);
            if (distance == 0)      // initial URLs should be downloaded
                seurl.setWeight(1000);
